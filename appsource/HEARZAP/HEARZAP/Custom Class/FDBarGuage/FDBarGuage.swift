...
@IBDesignable open class FDBarGauge: UIView {
    ...
    @IBInspectable open var maxLimit = 100.0
    ...
    @IBInspectable open var warnThreshold = 30.0 {
        didSet {
    ...
    /// A quantity for `value` which will render in a special color
    @IBInspectable open var dangerThreshold = 60.0 {
        didSet {
    ...
    @IBInspectable open var numBars: Int = 100 {
        didSet {
    ...
    @IBInspectable open var normalColor = UIColor.colorLiteral.theme_red_FF6961
    
    /// The rendered segment color after reaching the warning threshold
    @IBInspectable open var warningColor = UIColor.colorLiteral.theme_red_FF6961
    
    /// The rendered segment color after reaching the danger threshold
    @IBInspectable open var dangerColor = UIColor.colorLiteral.theme_red_FF6961
    
    fileprivate var onIdx = 0
    fileprivate var offIdx = 0
    fileprivate var peakBarIdx = -1
    fileprivate var warningBarIdx = 60
    fileprivate var dangerBarIdx = 80
    ...
    override open func draw(_ rect: CGRect) {
        ...
        rectBounds = CGRect(x: 0, y: 0, width: self.bounds.width, height: self.bounds.height)
        let isVertical: Bool = (rectBounds.size.height >= rectBounds.size.width)
        if isVertical {
            // Adjust height to be an exact multiple of bar
            barSize = rectBounds.size.height / CGFloat(numBars)
            rectBounds.size.height = barSize * CGFloat(numBars)
        } else {
            // Adjust width to be an exact multiple
            barSize = rectBounds.size.width / CGFloat(numBars)
            rectBounds.size.width = barSize * CGFloat(numBars)
        }
        // Compute size of bar
        ...
        for iX in 0..<numBars {
            ...
            if reverseDirection {
                ...
                rectBar.origin.x = (isVertical) ? rectBounds.origin.x + 1 : (rectBounds.maxX - CGFloat(CGFloat(iX + 1) * barSize))
                rectBar.origin.y = (isVertical) ? (rectBounds.minY + CGFloat(CGFloat(iX) * barSize)) : rectBounds.origin.y + 1
            } else {
                // Bottom-to-top or right-to-left
                rectBar.origin.x = (isVertical) ? rectBounds.origin.x + 1 : (rectBounds.minX + CGFloat(CGFloat(iX) * barSize))
                rectBar.origin.y = (isVertical) ? (rectBounds.maxY - CGFloat(CGFloat(iX + 1) * barSize)) : rectBounds.origin.y + 1
            }
