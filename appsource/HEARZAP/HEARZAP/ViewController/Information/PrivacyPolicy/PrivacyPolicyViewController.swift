...
class PrivacyPolicyViewController: UIViewController {
    ...
    func changeLanguageFormat() {
        self.titleLabel.text = AppLocalizationString.privacyPolicy
        if !(self.updatedOn.isEmpty) {
            //            self.descriptionLabel.text = self.updatedOn
            self.descriptionLabel.text = ""
        }
        self.titleViewHeight.constant =  20 + self.heightForText(text: AppLocalizationString.privacyPolicy, Font: AppFont.bold.size(22), Width: AppConstantValue.screenWidthminus32) + 4 + self.heightForText(text: self.updatedOn, Font: AppFont.medium.size(13), Width: AppConstantValue.screenWidthminus32) + 16
    ...
    func commonAPICALL(retryAPIID: Int) {
        DispatchQueue.main.async {
            ...
            if Reachability.isConnectedToNetwork() {
                if retryAPIID == 1000 {
                    self.getPrivacyPolicyData(retryAPIID: retryAPIID, canShowLoader: true)
                }
            } else {
    ...
    func getPrivacyPolicyData(retryAPIID: Int, canShowLoader: Bool) {
        self.privacyPolicyViewModel.getPrivacyPolicyData(canShowLoader: canShowLoader, viewCon: self, subAPIUrl: SubAPIUrl.getPrivacyPolicyData, methodName: HTTPMethods.get, parameter: "", pageNumber: "", retryAPIID: retryAPIID, strQuery: "")
    }
}
extension PrivacyPolicyViewController: ProtocolLogoNavigationBar {
    // MARK: - Navigation Bar Delegates
    func leftButtonPressed() {
        if self.isFrom == IsNavigateFrom.privacyPolicyMenu.rawValue {
...
extension PrivacyPolicyViewController: ProtocolNetworkRechabilityDelegate {
    // MARK: - InternetError
