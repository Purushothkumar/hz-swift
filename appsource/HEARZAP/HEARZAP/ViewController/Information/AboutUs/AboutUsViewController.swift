...
class AboutUsViewController: UIViewController {
    ...
    func changeLanguageFormat() {
        self.titleLabel.text = AppLocalizationString.aboutUs
        guard let version: String = Bundle.main.infoDictionary?["APP_VERSION"] as? String else { return }
        self.descriptionLabel.text = "\(AppLocalizationString.hearzapVersioncolon) \(version)"
        self.titleViewHeight.constant =  20 + self.heightForText(text: AppLocalizationString.aboutUs, Font: AppFont.bold.size(22), Width: AppConstantValue.screenWidthminus32) + 4 + self.heightForText(text: "\(AppLocalizationString.hearzapVersioncolon) \(version)", Font: AppFont.medium.size(13), Width: AppConstantValue.screenWidthminus32) + 16
    ...
    func commonAPICALL(retryAPIID: Int) {
        DispatchQueue.main.async {
            ...
            if Reachability.isConnectedToNetwork() {
                if retryAPIID == 1000 {
                    self.getAboutUsData(retryAPIID: retryAPIID, canShowLoader: true)
                }
            } else {
    ...
    func getAboutUsData(retryAPIID: Int, canShowLoader: Bool) {
        self.aboutUsViewModel.getAboutUsData(canShowLoader: canShowLoader, viewCon: self, subAPIUrl: SubAPIUrl.getAboutUsData, methodName: HTTPMethods.get, parameter: "", pageNumber: "", retryAPIID: retryAPIID, strQuery: "")
    }
}
extension AboutUsViewController: ProtocolLogoNavigationBar {
...
extension AboutUsViewController: ProtocolNetworkRechabilityDelegate {
    // MARK: - InternetError
