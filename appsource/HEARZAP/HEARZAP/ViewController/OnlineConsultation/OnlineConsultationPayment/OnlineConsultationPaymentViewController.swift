//
//  OnlineConsultationPaymentViewController.swift
//  HEARZAP
//
...
class OnlineConsultationPaymentViewController: UIViewController {
    // MARK: - IBOutlets
    ...
    @IBOutlet var audiologistNameLabel: SubheadFontLabel!
    ...
    // MARK: - Lets and Var
    var onlineConsultationPaymentViewModel = OnlineConsultationPaymentViewModel()
    ...
    func setFiveDotCircleView() {
        self.fiveDotCircleView.setupView(button1image: Asset.checkcircleblue, button2image: Asset.checkcircleblue, button3image: Asset.checkcircleblue, button4image: Asset.checkcircleblue, button5image: Asset.recordcircleblue,circleViewBackgroundColor: UIColor.colorLiteral.theme_yellow_FFFCEC)
    }
    // MARK: - Set Common Title View
    func setCommonTitleView() {
        let titlemessage = AppLocalizationString.OCStep5Title.capitalized
        let descriptionMessage = AppLocalizationString.OCStep5Desc.capitalized
    ...
    func changeLanguageFormat() {
        ...
        let strText1 = AppLocalizationString.bycontinuingyouhavereadandagreetothe
    ...
    func commonAPICALL(retryAPIID: Int) {
        DispatchQueue.main.async {
            ...
            if Reachability.isConnectedToNetwork() {
                if retryAPIID == 1000 {
                    self.postOnlineConsultationMakePayment(retryAPIID: retryAPIID, canShowLoader: true)
                } else if retryAPIID == 1001 {
                    self.postOnlineConsultationVerifyPayment(retryAPIID: retryAPIID, canShowLoader: true)
    ...
    func postOnlineConsultationMakePayment(retryAPIID: Int, canShowLoader: Bool) {
        let url = "\(SubAPIUrl.postOnlineConsultationMakePayment)\(bookingUid)/"
        self.onlineConsultationPaymentViewModel.postOnlineConsultationMakePayment(canShowLoader: canShowLoader, viewCon: self, subAPIUrl: url, methodName: HTTPMethods.post, parameter: "", pageNumber: "", retryAPIID: retryAPIID, strQuery: "")
    }
    func postOnlineConsultationVerifyPayment(retryAPIID: Int, canShowLoader: Bool) {
        let url = "\(SubAPIUrl.postOnlineConsultationVerifyPayment)\(bookingUid)/"
        ...
        self.onlineConsultationPaymentViewModel.postOnlineConsultationVerifyPayment(canShowLoader: canShowLoader, viewCon: self, subAPIUrl: url, methodName: HTTPMethods.post, parameter: param, pageNumber: "", retryAPIID: retryAPIID, strQuery: "")
    }
    func getBookingSummary(retryAPIID: Int, canShowLoader: Bool) {
        let url = "\(SubAPIUrl.getBookingSummary)\(bookingUid)/"
        self.onlineConsultationPaymentViewModel.getBookingSummary(canShowLoader: canShowLoader, viewCon: self, subAPIUrl: url, methodName: HTTPMethods.get, parameter: "", pageNumber: "", retryAPIID: retryAPIID, strQuery: "")
    }
    func applyCoupon(retryAPIID: Int, canShowLoader: Bool) {
        ...
        self.onlineConsultationPaymentViewModel.applyCoupon(canShowLoader: canShowLoader, viewCon: self, subAPIUrl: url, methodName: HTTPMethods.post, parameter: param, pageNumber: "", retryAPIID: retryAPIID, strQuery: "")
    }
    // MARK: - IBAction
    ...
    @objc func tappedOnLabel(_ gesture: UITapGestureRecognizer) {
        ...
        if gesture.didTapAttributedTextInLabel(label: self.privacyLabel, inRange: termsandConditionsRange) {
            self.navigateToTermsConditionsViewController(isFrom: IsNavigateFrom.termsofUseOCPayment.rawValue)
...
extension OnlineConsultationPaymentViewController: ProtocolTitleNavigationBar {
...
extension OnlineConsultationPaymentViewController: ProtocolNetworkRechabilityDelegate {
    // MARK: - InternetError
...
extension OnlineConsultationPaymentViewController: RazorpayPaymentCompletionProtocolWithData {
...
extension OnlineConsultationPaymentViewController: UITableViewDelegate, UITableViewDataSource {
