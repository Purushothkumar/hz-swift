//
//  OnlineConsultationUserInfoViewController.swift
//  HEARZAP
//
...
class OnlineConsultationUserInfoViewController: UIViewController,UITextFieldDelegate {
    // MARK: - IBOutlets
    ...
    var concernID = ""
    var onlineConsultationUserInfoViewModel = OnlineConsultationUserInfoViewModel()
    ...
    func setCommonTitleView() {
        let titlemessage = AppLocalizationString.OnlineConsultationStep3Title.uppercased()
        let descriptionMessage = AppLocalizationString.OnlineConsultationStep3Desc
    ...
    func commonAPICALL(retryAPIID: Int) {
        DispatchQueue.main.async {
            ...
            if Reachability.isConnectedToNetwork() {
                if retryAPIID == 1000 {
                    self.postOnlineConsultationCustomerDetail(retryAPIID: retryAPIID, canShowLoader: true)
                }
            } else {
    ...
    func postOnlineConsultationCustomerDetail(retryAPIID: Int, canShowLoader: Bool) {
        ...
        let strurl = String(format: "%@%@/", SubAPIUrl.postOnlineConsultationCustomerDetail,self.concernID)
        self.onlineConsultationUserInfoViewModel.postOnlineConsultationCustomerDetail(canShowLoader: canShowLoader, viewCon: self, subAPIUrl: strurl, methodName: HTTPMethods.post, parameter: param, pageNumber: "", retryAPIID: retryAPIID, strQuery: "")
    }
    // MARK: - IBAction
...
extension OnlineConsultationUserInfoViewController: ProtocolTitleNavigationBar {
...
extension OnlineConsultationUserInfoViewController: ProtocolNetworkRechabilityDelegate {
    // MARK: - InternetError
