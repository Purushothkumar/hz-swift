//
//  CareMaintenanceDetailViewController.swift
//  HEARZAP
//
...
class CareMaintenanceDetailViewController: UIViewController, WKNavigationDelegate {
    ...
    @IBOutlet var contentDataView: UIView!
    ...
    // MARK: - Lets and Var
    var careMaintenanceDetailViewModel = CareMaintenanceDetailViewModel()
    var listArray = [CareMaintenanceDetailModelCareAndMaintenance]()
    var slug = ""
    var htmlstring = ""
    ...
    override func viewWillLayoutSubviews() {
        ...
        if self.listArray.count > 0 {
            self.listTableViewHeight.constant = listTableView.contentSize.height
    ...
    func setNavigationBar() {
        ...
        self.navigationBar.title = AppLocalizationString.careambermaintenance
    ...
    func getCareMaintenanceList(retryAPIID: Int, canShowLoader: Bool) {
        if !(self.slug.isEmpty) {
            let suburl = "\(SubAPIUrl.getCareMaintenanceDetailList)\(slug.allowSpecialCharacters)/"
            self.careMaintenanceDetailViewModel.getCareMaintenanceDetailList(canShowLoader: canShowLoader, viewCon: self, subAPIUrl: suburl, methodName: HTTPMethods.get, parameter: "", pageNumber: "", retryAPIID: retryAPIID, strQuery: "")
        }
    }
...
extension CareMaintenanceDetailViewController: ProtocolTitleNavigationBar {
...
extension CareMaintenanceDetailViewController: ProtocolNetworkRechabilityDelegate {
    // MARK: - InternetError
...
extension CareMaintenanceDetailViewController: UITableViewDelegate, UITableViewDataSource {
    // MARK: - TableView Delegate
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.listArray.count
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: TableViewCellID.CareMaintenanceDetailTableViewCell, for: indexPath) as! CareMaintenanceDetailTableViewCell
        cell.configure(careMaintenanceDetailModel: self.listArray[indexPath.row], indexpath: indexPath.row)
        cell.delegate = self
        return cell
    }
...
extension CareMaintenanceDetailViewController: ProtocolCareMaintenanceDetailTableViewCell {
