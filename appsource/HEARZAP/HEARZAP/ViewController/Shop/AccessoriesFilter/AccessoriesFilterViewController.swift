//
//  AccessoriesFilterViewController.swift
//  HEARZAP
//
...
protocol ProtocolAccessoriesFilterViewController {
...
class AccessoriesFilterViewController: UIViewController {
    // MARK: - IBOutlets
    ...
    // MARK: - Lets and Var
    var accessoriesFilterViewModel = AccessoriesFilterViewModel()
    ...
    var delegate: ProtocolAccessoriesFilterViewController?
    ...
    func getAccessoriesFilterList(retryAPIID: Int, canShowLoader: Bool) {
        self.accessoriesFilterViewModel.getAccessoriesFilterList(canShowLoader: canShowLoader, viewCon: self, subAPIUrl: SubAPIUrl.getAccessoriesFilterList, methodName: HTTPMethods.get, parameter: "", pageNumber: "", retryAPIID: retryAPIID, strQuery: "")
    }
    // MARK: - IBAction
        ...
        for (index, item) in featureArraySelectedCheck.enumerated() {
            for (key, value) in (item as! NSDictionary) {
                if (key as! String) == strKeyCode {
                    ...
                    dicTemp.setValue(arrayValueCode, forKey: strKeyCode)
                    self.featureArraySelected.insert(dicTemp, at: index)
                    self.reloadFeatureTableView()
                    self.setUserInteractionButton()
                    return
                }
        ...
        for (index, item) in specificationArraySelectedCheck.enumerated() {
            for (key, value) in (item as! NSDictionary) {
                if (key as! String) == strKeyCode {
                    ...
                    let dicTemp = NSMutableDictionary()
                    dicTemp.setValue(arrayValueCode, forKey: strKeyCode)
                    self.specificationArraySelected.insert(dicTemp, at: index)
                    self.reloadSpecificationTableView()
                    self.setUserInteractionButton()
                    return
                }
    ...
    @IBAction func clearAllButtonPressed(_ sender: UIButton) {
        ...
        if self.featureArray.count > 0 {
            for item in self.featureArray {
                ...
                if !(keyCode.isEmpty) {
                    let dicTemp = NSMutableDictionary()
                    dicTemp.setValue([], forKey: keyCode)
                    self.featureArraySelected.add(dicTemp)
                }
            }
        }
        if self.specificationArray.count > 0 {
            for item in self.specificationArray {
                ...
                if !(keyCode.isEmpty) {
                    let dicTemp = NSMutableDictionary()
                    dicTemp.setValue([], forKey: keyCode)
                    self.specificationArraySelected.add(dicTemp)
                }
            }
        }
        self.setStyleUI()
    ...
    @IBAction func doneButtonPressed(_ sender: UIButton) {
        self.dismiss(animated: true) {
            self.delegate!.doneButtonPressedAccessoriesFilterViewController(styleArraySelected: self.styleArraySelected, brandArraySelected: self.brandArraySelected, featureArraySelected: self.featureArraySelected, specificationArraySelected: self.specificationArraySelected)
        }
    }
    // MARK: - Custom Methods
    func setStyleUI() {
...
extension AccessoriesFilterViewController: ProtocolNetworkRechabilityDelegate {
    // MARK: - InternetError
...
extension AccessoriesFilterViewController: UITableViewDelegate, UITableViewDataSource {
