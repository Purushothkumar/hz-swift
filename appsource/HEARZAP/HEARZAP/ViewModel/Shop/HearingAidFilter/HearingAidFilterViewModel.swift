//
//  HearingAidFilterViewModel.swift
//  HEARZAP
//
...
struct HearingAidFilterViewModel {
    func getHearingAidFilterList(canShowLoader: Bool, viewCon: HearingAidFilterViewController, subAPIUrl: String, methodName: String, parameter: String?, pageNumber: String?, retryAPIID: Int, strQuery: String?) {
        APIManager.shared.request(canShowLoader: canShowLoader, viewCon: viewCon, subAPIUrl: subAPIUrl, methodName: methodName, parameter: parameter, pageNumber: pageNumber, retryAPIID: retryAPIID, strQuery: strQuery) { result in
            do {
                let json = try JSONDecoder().decode(HearingAidFilterModel.self, from: result as Data)
                if json.data != nil {
                    ...
                    if viewCon.specificationArray.count > 0 {
                        ...
                        for item in viewCon.specificationArray {
                            if !(item.id.isNilOrEmpty) {
                                viewCon.specificationTableHeight += (50 + 20)
                            }
                            if item.value != nil {
                                if item.value!.count > 0 {
                                    viewCon.specificationTableHeight += (item.value!.count * 50)
                                }
                            }
                        }
